...
const DEFAULT_TEXT = 'Bhavya Mehta';

type TriggerMode = 'mount' | 'hover' | 'scroll' | 'loop';
...
const EntryAnimation = ({
  ...
  const viewBox = box
    ...
    : `0 ${-Number(fontSize)} 600 ${Number(fontSize) * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': `${Math.round(Number(fontSize) * 1.3)}px` } as React.CSSProperties}
      role="img"
